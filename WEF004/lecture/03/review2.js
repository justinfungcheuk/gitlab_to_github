...
class Player {
    ...
    attack(monster) {
        ...
        monster.injure(this.strength * multiplier, player); // player擺放在 injure 代表是由該 player 去攻擊 monster
...
class RareMonster extends Monster {
    ...
    //因為 exp 會給予 player
    constructor(exp, hp = 150) { // 要 specific hp 再 specific exp
    ...
    injure(strength, player) { // player 獲得 exp
        ...
        if (this.hp == 0) {
            player.gainExperience(monster.exp);
        }
    }
}
