...
//以下寫法不會重複出現alert('hi!'), 因為當我for (let element of document.querySelectorAll('.hi')) {時, return的就只有@_@的element存在, 而後加上去的element.innerHTML = '<div class="hi"> Merry Christmas!</div> ';是未出現的, 所以下列for的所有程式就不會發生該新加上去的element.innerHTML = '<div class="hi"> Merry Christmas!</div> '如果你要加該寫法, 就必須於其出世後才可加上去.
for (let element of document.querySelectorAll('.hi')) {
    element.addEventListener('click', function() {
        alert('hi!')
    });
